...
#define ADDR_CHAN_REG(reg, chan) (s626ptr->private->io_base + (reg) + (chan) * 8)
...
#define encpriv (    (struct enc_private *)   &(( a4l_get_subd(s626ptr->dev, 5) )->priv)  )
...
struct enc_private {
	...
	uint16_t (*get_enable)(struct s626_struct * s626ptr, struct enc_private *);
	uint16_t (*get_int_src)(struct s626_struct * s626ptr, struct enc_private *);
	uint16_t (*get_load_trig)(struct s626_struct * s626ptr, struct enc_private *);
	...
	void (*reset_cap_flags)(struct s626_struct * s626ptr, struct enc_private *);
